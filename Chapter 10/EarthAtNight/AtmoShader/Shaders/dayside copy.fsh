...
{       
    ...
    //the numbers here are purly arbitrary. They just happen to look pretty
    //good.
    
//    if(cloudColor[0]>1.15)

    if(0)
    {
        cloudColor[3]=1.0;
        gl_FragColor=(cloudColor*1.3+surfaceColor*.3+finalSpecular)*colorVarying;
    }
    else
        gl_FragColor = surfaceColor*colorVarying+finalSpecular*colorVarying;
        

           gl_FragColor = finalSpecular*colorVarying;
