...
- (id) init:(GLint)stacks slices:(GLint)slices radius:(GLfloat)radius squash:(GLfloat) squash textureFile:(NSString *)textureFile		//chapter 5
{
    ...
    //used during testing for teh number of extra spheres rendered after the first one
    //to test dataloading. This value shows up as the "batch" metric in Instruments.
    
    m_NumberBatchedSpheres=10;			
...
{		
    ...
    if(m_TextureInfo!=NULL)
        effect.texture2d0.name=m_TextureInfo.name;
